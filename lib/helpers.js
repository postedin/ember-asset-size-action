...
`;
  data.forEach((item) => {
    table += `${item.file}|${prettyBytes(item.raw, { signed: true })}${item.absolute ? ' (' + prettyBytes(item.absolute.raw) + ')' : ''}|${prettyBytes(item.gzip, { signed: true })}${item.absolute ? ' (' + prettyBytes(item.absolute.gzip) + ')' : ''}\n`;
  });
